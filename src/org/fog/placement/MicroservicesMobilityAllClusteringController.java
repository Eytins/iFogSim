 ...
 */
public class MicroservicesMobilityAllClusteringController extends MicroservicesController {
     ...
     * @param applications
     */
    public MicroservicesMobilityAllClusteringController(String name, List<FogDevice> fogDevices, List<Sensor> sensors, List<Application> applications, List<Integer> clusterLevel, Double clusterLatency, int placementLogic, LocationHandler locator) {
    ...
    public MicroservicesMobilityAllClusteringController(String name, List<FogDevice> fogDevices, List<Sensor> sensors, List<Application> applications, List<Integer> clusterLevel, Double clusterLatency, int placementLogic, Map<Integer, List<FogDevice>> monitored, LocationHandler locator) {
        ...
        for (int i = 0; i < size - 1; i++) {
            for (int j = i + 1; j < size; j++) {
                if (distanceMatrix[i][j] < Config.CLUSTER_DISTANCE_THRESHOLD) {
                    ...
                    deviceI.getClusterMembersToLatencyMap().put(deviceJ.getId(),
                            distanceMatrix[i][j] + random.nextDouble() * 5);
                    deviceJ.getClusterMembers().add(deviceI.getId());
                    deviceJ.getClusterMembersToLatencyMap().put(deviceI.getId(),
                            distanceMatrix[i][j] + random.nextDouble() * 5);
                }
            }
        }
